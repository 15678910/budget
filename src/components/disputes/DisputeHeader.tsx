...
export function DisputeHeader({ dispute }: { dispute: Dispute }) {
  ...
  return (
    <header>
      ...
      <h1 className="mt-3 text-3xl font-bold text-foreground md:text-4xl">{dispute.title}</h1>
        ...
        {dispute.figures.map((f) => (
          <div key={f.label} className="rounded-lg border border-border bg-muted/30 p-4">
            <dt className="text-base text-muted-foreground">
              {f.label}{' '}
              <span className="font-mono text-sm">{FIGURE_KIND_LABEL[f.kind]}</span>
            </dt>
            <dd className="mt-1 text-2xl font-bold tabular-nums text-foreground">{f.value}</dd>
            {f.note && (
              <dd className="mt-2 text-base leading-relaxed text-muted-foreground">{f.note}</dd>
            )}
            <dd className="mt-2 font-mono text-sm text-muted-foreground">출처 — {f.source}</dd>
          </div>
        ))}
      </dl>
    </header>
