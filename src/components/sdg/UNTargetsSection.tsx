...
export default function UNTargetsSection({ targets, meta }: UNTargetsSectionProps) {
  ...
  return (
    <div className="mt-3 border-t border-slate-100 pt-3">
      ...
      <p className="mt-1 inline-block rounded bg-sky-50 px-1.5 py-0.5 text-[10px] text-sky-700">
        UN SDG API · 영문 공식 · 국문 비공식 참고
      </p>
      <ul className="mt-1.5 space-y-2">
        {targets.map((t) => (
          <li key={t.code} className="text-xs leading-relaxed">
            <div className="flex gap-2">
              <span className="shrink-0 rounded bg-sky-100 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-sky-700">
                {t.code}
              </span>
